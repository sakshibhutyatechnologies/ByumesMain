...
router.post('/convert', upload.single('doc'), async (req, res) => {
  ...
  try {
    ...
    const activities = extractSteps(text);
...
function extractSteps(text) {
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      current = { step: parseInt(m[1], 10), activity: m[2] || '' };
    } else if (current) {
      current.activity += (current.activity ? ' ' : '') + line;
    }
  }
  if (current) steps.push(current);
  ...
  return steps.map(s => {
    ...
    const block = {
      step: s.step,
      activity: { en: instr, hi: instr, es: instr, de: instr, fr: instr }
    };
